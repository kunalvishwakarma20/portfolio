...
const DEFAULT_GLOW_COLOR = '23, 37, 209';
...
const About = () => {
  ...
  const ContactCard = () => {
    ...
    return (
      <motion.div 
        ...
        className="magic-bento-card magic-bento-card--border-glow relative col-span-2 row-span-1 p-6 rounded-2xl bg-[#000000] border border-white/10 overflow-hidden flex flex-col items-center justify-center"
        variants={item}
        style={{
  ...
  // Hide custom cursor when hovering over bento grid with smooth transitions
  useEffect(() => {
    const cursorCanvas = document.getElementById('fluid');
    ...
    const handleMouseEnter = () => {
      ...
      if (!state.isHiding) {
        state.isHiding = true;
        cursorCanvas.style.transition = 'opacity 0.4s cubic-bezier(0.4, 0, 0.2, 1), visibility 0.4s cubic-bezier(0.4, 0, 0.2, 1)';
        cursorCanvas.style.opacity = '0';
        // Delay visibility change to allow opacity transition
    ...
    const handleMouseLeave = () => {
      ...
      if (state.isHiding) {
        ...
        // Small delay to ensure visibility is set before opacity transition
    ...
    const setupEventListeners = () => {
      ...
      const bentoCards = document.querySelectorAll('#about .magic-bento-card');
      ...
      return { aboutSection, bentoCards };
    };

    // Setup initial listeners
    const { aboutSection, bentoCards } = setupEventListeners();

    // Also listen for when cards are added dynamically
    const observer = new MutationObserver(() => {
      const newCards = document.querySelectorAll('#about .magic-bento-card');
      newCards.forEach(card => {
        if (!card.hasAttribute('data-cursor-listener')) {
          card.setAttribute('data-cursor-listener', 'true');
          card.addEventListener('mouseenter', handleMouseEnter);
          card.addEventListener('mouseleave', handleMouseLeave);
        }
      });
  ...
  return (
    <section className="c-space section-spacing" id="about" ref={gridRef} style={{ isolation: 'isolate' }}>
      ...
      >
        ...
        <motion.div 
          ref={profileCardRef}
          className="magic-bento-card magic-bento-card--border-glow relative col-span-3 row-span-1 p-6 rounded-2xl bg-[#000000] border border-white/10 overflow-hidden"
          variants={item}
        >
          <motion.img
        ...
        <motion.div 
          ref={floatingCardRef}
          className="magic-bento-card magic-bento-card--border-glow relative col-span-3 row-span-2 p-6 rounded-2xl bg-[#000000] border border-white/10 overflow-hidden"
          variants={item}
        >
          <motion.div
        ...
        <motion.div 
          ...
          className="magic-bento-card magic-bento-card--border-glow relative col-span-4 row-span-1 p-6 rounded-2xl bg-[#000000] border border-white/10 overflow-hidden"
          variants={item}
        >
